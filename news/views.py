...
class ArticleDetail(View):
    ...
    def get(self, request, slug, *args, **kwargs):
        ...
        if article.likes.filter(id=request.user.id).exists():
            liked = True
        # Render the article detail view with the "article-detail.html"
        # template
        return render(
            request,
            "article_detail.html",
...
class ArticleLike(View):
    ...
    def post(self, request, slug, *args, **kwargs):
        ...
        if article.likes.filter(id=request.user.id).exists():
            ...
            messages.success(
                request, "Thank you for giving this article a like!"
            )
...
class SearchResults(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Create search query:
        # Find any article that has all the keywords in either
        # the title, location, or content
        search_query = reduce(
            operator.and_,
            (
                Q(
        ...
        # Render the search results view with the "search-results.html"
        # template
        return render(
            request,
            "search-results.html",
            {"search_results": current_page, "keywords": keyword_list},
        )
